...
module.exports = function plugin(schema, options) {
  ...
  async function onUpdate(operation) {
    ...
    if (doc && doc.configs && doc.configs.$set) {
      _.forOwn(doc.configs.$set, (v, k) => {
        _.set(doc, k, v);
      });
      delete doc.configs.$set;
    }
...
let docGet = (doc, path) => (doc && _.isFunction(doc.get) ? doc.get(path) : _.get(doc, path));
const docSet = (doc, path, val) =>
  doc && _.isFunction(doc.set) ? doc.set(path, val) : _.set(doc, path, val);
const docIsModified = (doc, path) =>
  doc && _.isFunction(doc.isModified) ? doc.isModified(path) : _.get(doc, path) !== undefined;
